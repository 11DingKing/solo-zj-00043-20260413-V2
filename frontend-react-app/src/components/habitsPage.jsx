...
import {
  fetchGetHabits,
  fetchHabitCompletion,
  fetchUncompleteHabit,
  fetchGetHabitsByCategory,
} from "../api_fetching/urlParserMainFucntionality";
...
export const Habits = () => {
    ...
    const fetchHabits = async () => {
      try {
        ...
        try {
          ...
          const errorFlag = handleResponseError(
            response,
            responseJSON,
            navigate,
            setToken,
        ...
        try {
          ...
          for (let i = 0; i < responseJSON.length; i++) {
            ...
            const timeString = await getClosestResetTime(
              habit.reset_at,
              habit.completed,
            );
  ...
  const checkboxHandler = async (e, habitID, index) => {
    ...
    try {
      ...
      console.error(err);
      navigate("/internal-server-error", {
        state: { errorMessage: "Server down, try again later. " },
      });
      return;
    }
  };
                      ...
                      (habit, index) =>
                        habit && (
                          <div key={index}>
                            ...
                            >
                              <div>
                                <h3 className="text-xl font-bold mb-2 truncate text-gray-900 dark:text-white">
                                  {habit.habit_name}
                                </h3>
                                <p className="text-base mb-2 break-words text-gray-700 dark:text-white">
                                  {habit.habit_desc}
                                </p>
                              ...
                              <div>
                                <DeleteHabit
                                  ...
                                  habits={habits}
                                  index={index}
                                  setHabitsNumber={setHabitsNumber}
                                  habitsNumber={habitsNumber}
                                />
                              </div>
                            </div>
                          </div>
